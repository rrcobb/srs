...
impl Quality {
    pub fn weighted_random() -> Quality {
        let mut rng = thread_rng();
        let weights = [2,4,3,1,1,1];
        let dist = WeightedIndex::new(&weights).unwrap();
        let items = [Five, Four, Three, Two, One, Zero];
...
pub fn sm2(quality: Quality, previous_ease_factor: EaseFactor, previous_repetitions: Repetitions, previous_interval: Interval) -> (Interval, Repetitions, EaseFactor) {
...
impl Fact {
    pub fn default(name: String) -> Fact {
        Fact {
            name,
    ...
    pub fn update(old: Fact, quality: Quality) -> Fact {
        let (new_interval, new_repetitions, new_ease_factor) = sm2(quality, old.ease_factor, old.repetitions, old.interval);
        Fact {
            name: old.name,
